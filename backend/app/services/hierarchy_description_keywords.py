...
_GENERIC_SINGLE_RE = re.compile(r"^(?:khái|niệm|liên|quan|thuộc|nội|dung|chính|kiến|thức|mạng|tính|bài|mục|chủ|đề)$", re.I)
_SINGLE_WORD_ALLOWLIST = {
    "internet", "byte", "bit", "wifi", "sql", "html", "css", "python",
    "scratch", "linux", "windows", "word", "excel", "powerpoint", "google",
    "facebook", "youtube", "email", "tcp", "udp", "ip", "dns", "web"
}
...
def _trim_generic_phrase(text: str) -> str:
    ...
    low = out.casefold()
    for part in GENERIC_PHRASE_PARTS:
        idx = low.find(part)
        if idx > 0:
...
def _normalize_keyword_candidate(value: Any) -> str:
    ...
    if low_joined in {"nội dung", "kiến thức", "mạng máy tính và internet là nội dung"}:
        return ""
...
def _fallback_summary(
    ...
    *,
    ...
    child_names = _uniq_keep_order([_clean(item.get(child_name_key)) for item in (children or []) if _clean(item.get(child_name_key))], limit=12)
    ...
    if _clean(existing_description):
        ...
        if level == "subject":
            description = f"{clean_name} bao quát toàn bộ cuốn sách/môn học, gồm các mảng nội dung chính như: {', '.join(child_names[:10])}."
        elif level == "topic":
...
def _build_prompt(level: str, name: str, context_text: str, keyword_limit: int) -> Tuple[str, int]:
    if level == "lesson":
        prompt = (
            "Bạn đang xây dựng hệ thống tìm kiếm học liệu. Hãy đọc phần ngữ cảnh tổng hợp của BÀI HỌC và viết mô tả bao quát giống như khi làm cho chunk nhưng ở cấp bài. "
            "Mô tả cần tóm được các khái niệm chính, định nghĩa, quy trình, ví dụ, ứng dụng và các phần quan trọng trong bài. "
    ...
    if level == "topic":
        prompt = (
            "Bạn đang xây dựng hệ thống tìm kiếm học liệu. Hãy đọc phần ngữ cảnh tổng hợp của CHỦ ĐỀ và viết mô tả đủ chi tiết, đủ dài để bao quát toàn bộ chủ đề. "
            "Mô tả phải thể hiện được các bài/mục lớn, các khái niệm, quy trình, kỹ năng, ứng dụng và mối liên hệ giữa các phần trong chủ đề. "
            "Sau đó trích xuất thật đầy đủ các keyword/cụm keyword có nghĩa tìm kiếm trực tiếp, bao phủ toàn bộ các ý chính đã xuất hiện trong description. "
            "Ưu tiên cụm từ 2-6 từ, không lấy map id, không lấy token rời rạc. "
            f"Trả về JSON đúng schema {{'description':'...','keywords':['kw1','kw2']}} với tối đa {keyword_limit} keyword. "
        ...
        return prompt, 1400
    prompt = (
        "Bạn đang xây dựng hệ thống tìm kiếm học liệu. Hãy đọc phần ngữ cảnh tổng hợp của MÔN HỌC/CUỐN SÁCH và viết mô tả thật chi tiết, đủ dài, có tính bao hàm để đại diện cho toàn bộ cuốn sách. "
        "Mô tả cần bao quát phạm vi kiến thức, các nhóm chủ đề lớn, kỹ năng, ứng dụng và nội dung xuyên suốt toàn tài liệu. "
        "Sau đó trích xuất thật đầy đủ các keyword/cụm keyword có nghĩa tìm kiếm trực tiếp, bao phủ toàn bộ các ý chính đã xuất hiện trong description. "
        "Ưu tiên cụm từ 2-6 từ, không lấy map id, không lấy token rời rạc. "
        f"Trả về JSON đúng schema {{'description':'...','keywords':['kw1','kw2']}} với tối đa {keyword_limit} keyword. "
        "Không markdown. Không giải thích. "
...
def rebuild_hierarchy_descriptions_and_keywords(
    *,
    ...
    if lesson_doc:
        ...
        fallback_keywords = _merge_keywords(
            existing_lesson_keywords,
            _description_seed_keywords(_clean(lesson_doc.get("lessonDescription")), limit=_keyword_limit("lesson")),
            [kw for chunk in lesson_chunks for kw in _split_keywords(chunk.get("keywords"), limit=20)],
            limit=_keyword_limit("lesson"),
        )
    ...
    if topic_doc:
        ...
        fallback_keywords = _merge_keywords(
            existing_topic_keywords,
            _description_seed_keywords(_clean(topic_doc.get("topicDescription")), limit=_keyword_limit("topic")),
            [kw for lesson in topic_lessons for kw in _split_keywords(lesson.get("keywordLesson"), limit=20)],
            limit=_keyword_limit("topic"),
        )
        description, keywords, meta = _call_desc_keywords_api(
            level="topic",
    ...
    if subject_doc:
        ...
        existing_subject_keywords = _split_keywords(subject_doc.get("keywordSubject"), limit=_keyword_limit("subject"))
        context = _build_context_lines(
            "Subject",
        ...
        fallback_keywords = _merge_keywords(
            existing_subject_keywords,
            _description_seed_keywords(_clean(subject_doc.get("subjectDescription")), limit=_keyword_limit("subject")),
            [kw for topic in subject_topics for kw in _split_keywords(topic.get("keywordTopic"), limit=30)],
